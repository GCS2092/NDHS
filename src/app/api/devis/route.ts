...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, phone, service, description, budget, urgency } = body;
    ...
    return NextResponse.json({ success: true, message: 'Demande de devis envoyée avec succès' }, { status: 200 });
  } catch (error) {
