...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fullName, email, phone, destination, departureDate, returnDate, passengers, flightClass, customRequest, budget, flexibility, isCustom = false } = body;
    ...
    try {
      const sanityDoc = {
        ...
        fullName, email, phone, destination, departureDate, returnDate,
        passengers: parseInt(passengers),
        flightClass: flightClass || 'economique',
        customRequest,
        budget: budget ? parseInt(budget.replace(/\D/g, '')) : undefined,
        flexibility, isCustom, status: 'pending',
      };
    ...
    const { error } = await resend.emails.send({
      ...
      subject: isCustom ? `[Réservation Personnalisée] ${destination} - ${fullName}` : `[Réservation Billets] ${destination} - ${fullName}`,
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2563eb;">${isCustom ? 'Nouvelle réservation personnalisée' : 'Nouvelle réservation de billets d\'avion'}</h2>
        ...
        ${customRequest ? `<div style="background: #fff; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
    ...
    return NextResponse.json({ success: true, message: isCustom ? 'Réservation personnalisée envoyée avec succès' : 'Réservation de billets envoyée avec succès' }, { status: 200 });
  } catch (error) {
